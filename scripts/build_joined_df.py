...
def build_final_train_dataset(data_dir: Path, output_path: Optional[Path] = None) -> pd.DataFrame:
    """
    Construye el dataset final usando:
        data/raw/application_train
        data/raw/bureau
        data/raw/previous_application
    ...
    print(f"[INFO] Carpeta raw: {raw_dir}")
    print(f"[INFO] Carpeta trusted: {trusted_dir}")

    app = read_table(raw_dir, "application_train", required=True)
    app = clean_application_train(app)

    print(f"[INFO] Base application_train limpia: {app.shape[0]:,} filas x {app.shape[1]:,} columnas")

    # Cargar solo las tablas necesarias desde data/raw.
    bureau = read_table(raw_dir, "bureau", required=False)
    previous_application = read_table(raw_dir, "previous_application", required=False)
    ...
    if output_path is None:
        output_path = trusted_dir / "joined_df.parquet"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Ruta del parquet final. Default: "
            "data/trusted/joined_df.parquet"
        ),
    )
